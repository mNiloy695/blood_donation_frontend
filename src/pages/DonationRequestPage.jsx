...
const DonationRequestForm = () => {
  ...
  const [lastDonate, setLastDonate] = useState('');  // Set to empty string initially
  ...
  }, [navigation]);  // Only trigger on navigation change
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
     console.log(user,blood,address,phone,unit)
    // Validation
    if (!blood || !address || !phone || !unit) {
      setError('All fields are required.');
      return;
    }

    try {
      const response = await axios.post('https://blood-donation-backend-v3bp.vercel.app/donate/request/', 
      ...
      if (response.data.error) {
        ...
        setError('');
        setUnit('');
        setLastDonate('');  // Make sure this is reset to an empty string as well
        navigation('/doners');
      }
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="flex justify-center  items-center">
        <div className="flex flex-col md:flex-row shadow-md bg-[#ffff]">
          <div className="flex items-center justify-center w-[100%]">
            <img className="w-[90%]" src={BloodImage} alt="" />
          </div>
          {/* Form Section */}
          <div className="w-full p-8">
            <h2 className="text-3xl font-semibold text-center text-gray-800 mb-6 font-[Poppins]"> Request For Donation</h2>
