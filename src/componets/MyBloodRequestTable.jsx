  ...
  useEffect(() => {
    ...
    const fetchRequests = async () => {
      try {
        ...
        const myRequestForBlood=await axios.get(`https://blood-donation-backend-v3bp.vercel.app/recive/request/?user_id=${user}`)
  ...
  return (
   <>
    <div className="md:p-8 mt-5 md:mx-auto">
      <h1 className='m-5 flex items-center justify-center font-semibold'>Your Request's For Blood</h1>
          ...
          <tbody className="text-center">
            {myRequest.length === 0 ? (
              ...
              myRequest.map((request) => (
                <TableRow track="recive" key={request.id} id={request.id} address={request.address} phone={request.phone} status="Cancelled" unit={request.unit} blood={request.blood}/>
              ))
...
export default MyBloodRequestsTable